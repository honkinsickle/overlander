-- ============================================================================
-- CORRECTION to 20260901000200: remove operational_status from
-- v_clearable_fields.
--
-- 20260901000200 restored the clear-branch and, as a deliberate flagged
-- addition beyond a pure merge, put operational_status in the clearable list.
-- The reasoning was that it is a nullable precedence-resolved text column like
-- the other nine, so excluding it would leave it carrying the same clear-bug.
--
-- That reasoning is sound in principle and wrong in fact. MEASURED on TEST
-- immediately after applying 20260901000200:
--
--   select count(*) from source_record
--    where is_active and normalized_payload ? 'operational_status';   -->  0
--
-- Not one source_record carries the field in normalized_payload, which is the
-- only place resolve_field() looks. Every value in
-- master_place.operational_status was written DIRECTLY by
-- data/scripts/backfill-operational-status.ts, which reads
-- raw_payload->'props'->>'seasonal_operational_status' and updates
-- master_place. (6,324 active usfs records carry the RAW field; 0 carry the
-- normalized one.) So resolve_field() returns no candidate for every row, and
-- with operational_status in the clearable list every recompute erased it.
--
-- Caught by this branch's own verification step, not in review: recomputing a
-- row with operational_status = 'CLOSED' left it NULL. One row was lost that
-- way and has been restored by re-running the PR #321 backfill.
--
-- This is structurally the SAME defect as the one this branch exists to fix —
-- a value written directly into master_place, invisible to the function that
-- owns the column. Re-adding operational_status here is correct only once
-- normalized_payload.operational_status is backfilled. Filed in docs/BACKLOG.md.
--
-- Everything else in 20260901000200 is unchanged: all seven restored sites,
-- operational_status still resolved in v_jsonb_fields/v_text_columns (so newly
-- ingested USFS rows resolve normally), source_count still excludes generated
-- sources.
--
...
declare
  ...
  --
  -- operational_status is DELIBERATELY ABSENT, and this is the second attempt.
  -- 20260901000200 added it here on the reasoning that it is a nullable
  -- precedence-resolved text column like the other nine, so leaving it out
  -- would expose it to the very clear-bug this branch fixes. That reasoning is
  -- sound in principle and WRONG against the actual data: MEASURED on TEST
  -- 2026-09-01, ZERO source_records carry 'operational_status' in
  -- normalized_payload. Every value in master_place.operational_status was put
  -- there by a DIRECT WRITE (data/scripts/backfill-operational-status.ts, which
  -- reads source_record.raw_payload->'props'->>'seasonal_operational_status'
  -- and updates master_place). So resolve_field() cannot reproduce any of them,
  -- and listing the field here made every recompute erase it — the exact trap
  -- PR #327's description writes created, recreated for a different column.
  -- Caught by this branch's own verification (one row lost, restored).
  --
  -- Adding it back is correct ONLY after the values are resolvable, i.e. after
  -- normalized_payload.operational_status is backfilled for the usfs records
  -- that already carry the raw field. Filed in docs/BACKLOG.md.
