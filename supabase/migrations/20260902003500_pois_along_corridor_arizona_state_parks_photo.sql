-- Add 'arizona_state_parks' to pois_along_corridor's photo lateral.
--
-- AZ counterpart to 20260902000100 (OR oregon_state_parks). Priority
-- 10, since the parallel `nevada_state_parks` PR (on a different
-- branch) already applied its migrations to TEST and took slot 9. AZ
-- takes 10 and this CREATE OR REPLACE preserves NV at 9 in the
-- CASE / IN list so NV's photo pipeline on TEST is not broken.
--
-- MERGE NOTE: if NV's PR merges to main first, this migration layers
-- cleanly. If AZ merges first, NV's migration will need to bump AZ's
-- IN-list mention to preserve slot 10 during its own CREATE OR
-- REPLACE. Whichever PR merges second owns the reconciliation.
--
-- APPLY-PATH:
...
begin
  ...
  left join lateral (
    ...
    from public.source_record sr
    where sr.master_place_id = mp.id
      and sr.source_id in ('nps', 'ridb', 'wikipedia', 'atlas_oddities', 'family_destinations', 'editorial_food', 'state_parks_web', 'state_parks_web_wa', 'oregon_state_parks', 'nevada_state_parks', 'arizona_state_parks')
    ...
    order by case sr.source_id
                ...
                when 'arizona_state_parks' then 10
                else 11
             end
    limit 1
  ) n on true
...
comment on function public.pois_along_corridor(jsonb, integer, text[]) is
  'Phase 2 consumer corridor read. photo_url from nps/ridb/wikipedia/atlas_oddities/family_destinations/editorial_food/state_parks_web/state_parks_web_wa/oregon_state_parks/nevada_state_parks/arizona_state_parks via LEFT JOIN LATERAL, NPS preferred.';
