-- Add 'nevada_state_parks' to pois_along_corridor's photo lateral.
--
-- NV counterpart to OR's 20260902000100. Priority 9 (next after
-- oregon_state_parks at 8; slot ordering: nps→ridb→wikipedia→atlas
-- →family_destinations→editorial_food→state_parks_web→state_parks_web_wa
-- →oregon_state_parks→nevada_state_parks).
--
-- Photo credit rendered as "Nevada State Parks"; license label is also
-- "Nevada State Parks" (NOT the "— government publication" framing
-- CA/WA/OR use). parks.nv.gov carries no reuse grant text and nv.gov's
-- default is "All Rights Reserved" — this is Adam's explicit risk
-- acceptance, not a resolved license-clear determination. See BACKLOG.md.
--
-- APPLY-PATH:
...
comment on function public.pois_along_corridor(jsonb, integer, text[]) is
  'Phase 2 consumer corridor read. photo_url from nps/ridb/wikipedia/atlas_oddities/family_destinations/editorial_food/state_parks_web/state_parks_web_wa/oregon_state_parks/nevada_state_parks via LEFT JOIN LATERAL, NPS preferred.';
