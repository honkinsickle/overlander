...
left join lateral (
  ...
  where sr.master_place_id = mp.id
    and sr.source_id in ('nps', 'ridb', 'wikipedia', 'atlas_oddities', 'family_destinations', 'editorial_food', 'state_parks_web', 'state_parks_web_wa', 'oregon_state_parks', 'nevada_state_parks', 'arizona_state_parks', 'utah_state_parks')
...
comment on view public.master_place_search_export is
  'Phase 2 Typesense sync source. photo_url from nps/ridb/wikipedia/atlas_oddities/family_destinations/editorial_food/state_parks_web/state_parks_web_wa/oregon_state_parks/nevada_state_parks/arizona_state_parks/utah_state_parks via LEFT JOIN LATERAL, NPS preferred.';
