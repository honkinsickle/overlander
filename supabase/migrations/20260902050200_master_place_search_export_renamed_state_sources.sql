-- Re-issue public.master_place_search_export with the renamed source_ids.
--
-- Body is 20260902040300's definition verbatim except for two identifiers:
--   'state_parks_web'    -> 'california_state_parks'
--   'state_parks_web_wa' -> 'washington_state_parks'
-- Photo-lateral priority SLOTS are unchanged (CA stays 6, WA stays 7), so
-- no photo changes hero image as a result of this rename.
-- Companion to 20260902050000.

-- Add 'utah_state_parks' to master_place_search_export's photo lateral.
--
-- Mirrors 20260902040200 (pois_along_corridor). UT takes slot 11
-- after AZ's slot 10.
--
-- APPLY-PATH:
...
left join lateral (
  ...
  where sr.master_place_id = mp.id
    and sr.source_id in ('nps', 'ridb', 'wikipedia', 'atlas_oddities', 'family_destinations', 'editorial_food', 'california_state_parks', 'washington_state_parks', 'oregon_state_parks', 'nevada_state_parks', 'arizona_state_parks', 'utah_state_parks')
  ...
  order by case sr.source_id
    ...
    when 'california_state_parks' then 6
    when 'washington_state_parks' then 7
    when 'oregon_state_parks' then 8
    when 'nevada_state_parks' then 9
    when 'arizona_state_parks' then 10
    when 'utah_state_parks' then 11
    else 12
  end
  limit 1
) photo on true
...
comment on view public.master_place_search_export is
  'Phase 2 Typesense sync source. photo_url from nps/ridb/wikipedia/atlas_oddities/family_destinations/editorial_food/california_state_parks/washington_state_parks/oregon_state_parks/nevada_state_parks/arizona_state_parks/utah_state_parks via LEFT JOIN LATERAL, NPS preferred.';
