-- Re-issue public.pois_along_corridor with the renamed source_ids.
--
-- Body is 20260902040200's definition verbatim except for two identifiers:
...
-- Add 'utah_state_parks' to pois_along_corridor's photo lateral.
--
-- UT takes slot 11, after AZ's slot 10. The `else` catch-all bumps
-- to 12.
--
-- APPLY-PATH:
...
comment on function public.pois_along_corridor(jsonb, integer, text[]) is
  'Phase 2 consumer corridor read. photo_url from nps/ridb/wikipedia/atlas_oddities/family_destinations/editorial_food/california_state_parks/washington_state_parks/oregon_state_parks/nevada_state_parks/arizona_state_parks/utah_state_parks via LEFT JOIN LATERAL, NPS preferred.';
