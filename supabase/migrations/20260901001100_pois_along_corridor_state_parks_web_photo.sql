-- Add 'state_parks_web' to pois_along_corridor's photo lateral.
--
-- state_parks_web photos are curated hero images from parks.ca.gov
-- (California State Parks — government publication). Priority 6, after
-- editorial_food (5) — the same pattern as every prior photo-source
-- addition (family_destinations, editorial_food, atlas_oddities).
--
-- APPLY-PATH:
...
begin
  ...
  left join lateral (
    ...
    from public.source_record sr
    where sr.master_place_id = mp.id
      and sr.source_id in ('nps', 'ridb', 'wikipedia', 'atlas_oddities', 'family_destinations', 'editorial_food', 'state_parks_web')
