-- Add 'state_parks_web_wa' to pois_along_corridor's photo lateral.
--
-- WA counterpart to 20260901001100 (CA). Priority 7, after
-- state_parks_web (CA, 6). They never compete (different states).
...
begin
  ...
  left join lateral (
    ...
    from public.source_record sr
    where sr.master_place_id = mp.id
      and sr.source_id in ('nps', 'ridb', 'wikipedia', 'atlas_oddities', 'family_destinations', 'editorial_food', 'state_parks_web', 'state_parks_web_wa')
...
comment on function public.pois_along_corridor(jsonb, integer, text[]) is
  'Phase 2 consumer corridor read. nps_photo_url from nps/ridb/wikipedia/atlas_oddities/family_destinations/editorial_food/state_parks_web/state_parks_web_wa (NPS preferred). SECURITY DEFINER.';
