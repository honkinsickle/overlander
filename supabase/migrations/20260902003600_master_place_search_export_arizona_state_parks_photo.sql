-- Add 'arizona_state_parks' to master_place_search_export's photo lateral.
--
-- Mirrors 20260902000500 (pois_along_corridor). Priority 10 — NV
-- already took slot 9 on TEST via a parallel PR. AZ preserves NV at 9
-- in the CASE / IN list so NV's search-export photo pipeline is not
-- broken.
--
-- MERGE NOTE: same reconciliation applies as 20260902000500 — whichever
-- of the AZ / NV PRs lands second on main owns updating its own
-- CREATE OR REPLACE to preserve the other's slot.
--
-- APPLY-PATH:
...
comment on view public.master_place_search_export is
  'Phase 2 Typesense sync source. photo_url from nps/ridb/wikipedia/atlas_oddities/family_destinations/editorial_food/state_parks_web/state_parks_web_wa/oregon_state_parks/nevada_state_parks/arizona_state_parks via LEFT JOIN LATERAL, NPS preferred.';
