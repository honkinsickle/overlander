-- ============================================================================
-- Add 'wikipedia' to the photo lateral join on pois_along_corridor, and
-- surface photo_credit alongside the existing nps_photo_url.
--
-- Precedence: nps (0) > ridb (1) > wikipedia (2). The column is still
-- named nps_photo_url for backward compatibility with mapMasterPlaceRow
-- in federated.ts — it returns the winning photo URL regardless of source.
--
-- photo_credit: the credit string from normalized_payload.photo.credit
-- (e.g. "John Doe / CC BY-SA 4.0" for CC images, null for PD or NPS).
-- Surfaced so the card can render attribution for CC-licensed images.
--
...
-- RETURNS TABLE gains one column: photo_credit text.
--
...
begin
  ...
  left join lateral (
    ...
    from public.source_record sr
    where sr.master_place_id = mp.id
      and sr.source_id in ('nps', 'ridb', 'wikipedia')
      and sr.normalized_payload->'photo'->>'url' is not null
    order by case sr.source_id when 'nps' then 0 when 'ridb' then 1 when 'wikipedia' then 2 else 3 end
    limit 1
  ) n on true
  left join lateral (
    select
