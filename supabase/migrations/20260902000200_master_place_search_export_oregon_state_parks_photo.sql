-- Add 'oregon_state_parks' to master_place_search_export's photo lateral.
--
-- Mirrors 20260902000100 (pois_along_corridor). Priority 8, with slot 7
-- held by state_parks_web_wa (WA, wa-state-parks branch — already
-- applied to TEST via 20260901001500, not yet merged to main). Includes
-- state_parks_web_wa in the IN list to preserve WA across CREATE OR
-- REPLACE.
--
-- APPLY-PATH:
...
comment on view public.master_place_search_export is
  'Phase 2 Typesense sync source. photo_url from nps/ridb/wikipedia/atlas_oddities/family_destinations/editorial_food/state_parks_web/state_parks_web_wa/oregon_state_parks via LEFT JOIN LATERAL, NPS preferred.';
