...
- **Direct `psql` DDL against PROD bypasses the ledger** — an object created outside `db push` (or `db:push-verify`) leaves `supabase_migrations.schema_migrations` with no row, so `migration list` and any file-vs-ledger comparison won't see it. This happened 2026-08-10: `reference_trips.is_active` and `six_state_footprint()` reached PROD via a parallel Conductor worktree and were only reconciled later with `migration repair --status applied`. `bin/preflight` now flags PROD ledger versions that have **no local file**, but note it catches a missing *file*, not an object created with **no ledger row at all** — so applying DDL only through `db:push-verify` remains the real guard.
