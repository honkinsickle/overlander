...
describe("templates", () => {
  it("there are exactly 12", () => {
    expect(TEMPLATE_COUNT).toBe(12);
    expect(CAPTION_TEMPLATES).toHaveLength(12);
  });
});
...
describe("null state", () => {
  it("drops the state segment cleanly for ALL 12 templates (no dangling separator)", () => {
...
describe("buildCaption", () => {
  ...
  it("template 4 interpolates place/category/state exactly", () => {
    ...
    expect(c.text).toBe(
      'Most "hidden gems" are a parking lot with a sign. This one isn\'t. Boulder Basin. Campground · CA. Worth the stop. Get the next one in your inbox. Link in bio.',
    );
  });

  it("rejects out-of-range template numbers", () => {
    expect(() => buildCaption(candidate(), 0)).toThrow(/1-12/);
    expect(() => buildCaption(candidate(), 13)).toThrow(/1-12/);
  });
});
...
describe("pickLruTemplate", () => {
  ...
  it("never repeats the most recent template", () => {
    for (let last = 1; last <= 12; last++) {
      expect(pickLruTemplate([last])).not.toBe(last);
    }
  });
  it("round-robins through all 12 then cycles (LRU)", () => {
