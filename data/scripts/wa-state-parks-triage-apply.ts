 ...
 *
 * Run (TEST):
 *   npx tsx --env-file=.env scripts/wa-state-parks-triage-apply.ts
 *   npx tsx --env-file=.env scripts/wa-state-parks-triage-apply.ts --apply decisions.json
 *   npx tsx --env-file=.env scripts/wa-state-parks-triage-apply.ts --apply decisions.json --write
 */
...
runStateParksTriage(
  ...
  logger.error({ err: e }, "wa-triage: fatal");
  process.exit(1);
});
