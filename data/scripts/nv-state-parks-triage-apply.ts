/**
 * Manual-review triage for `nevada_state_parks`.
 *
 ...
 *
 * Run (TEST):
 *   npx tsx --env-file=.env scripts/nv-state-parks-triage-apply.ts
 *   npx tsx --env-file=.env scripts/nv-state-parks-triage-apply.ts --apply decisions.json
 *   npx tsx --env-file=.env scripts/nv-state-parks-triage-apply.ts --apply decisions.json --write
 */
...
runStateParksTriage(
  sb,
  {
    ...
    resolver: `adam:nv-triage-${new Date().toISOString().slice(0, 10)}`,
    label: "nv-triage",
  },
  (path) => JSON.parse(readFileSync(path, "utf8")) as unknown,
).catch((e: unknown) => {
  logger.error({ err: e }, "nv-triage: fatal");
  process.exit(1);
});
