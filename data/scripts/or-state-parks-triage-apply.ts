/**
 * Manual-review triage for `oregon_state_parks`.
 *
 ...
 *
 * Run (TEST):
 *   npx tsx --env-file=.env scripts/or-state-parks-triage-apply.ts
 *   npx tsx --env-file=.env scripts/or-state-parks-triage-apply.ts --apply decisions.json
 *   npx tsx --env-file=.env scripts/or-state-parks-triage-apply.ts --apply decisions.json --write
 */
...
runStateParksTriage(
  sb,
  {
    ...
    resolver: `adam:or-triage-${new Date().toISOString().slice(0, 10)}`,
    label: "or-triage",
  },
  (path) => JSON.parse(readFileSync(path, "utf8")) as unknown,
).catch((e: unknown) => {
  logger.error({ err: e }, "or-triage: fatal");
  process.exit(1);
});
