...
> **⚠ SUPERSEDED IN PART — read this first.** This document verifies the
> `resolvePlaces()` diagram *as it stood on 2026-09-03*. Two changes landed on
> `main` afterwards that alter what it describes:
>
> - **#398** — `search-area` and `trip-browse` were cut **fully** onto
>   `resolvePlaces()`, changing the built-vs-wired picture this pass recorded.
> - **#394** — Auto/Repair wired to live Mapbox (`auto_repair`, `car_wash`).
>
> Its "verified current state" and "as it stands today" claims are therefore
> **historical, not current**. Kept as a dated investigation record. The Paper
> artboard it describes has not been re-checked since and may carry the same
> drift.
