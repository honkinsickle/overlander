...
## 2026-08-28 (later) — editorial_food: multi-publisher road-trip food source (TEST end-to-end)

- **Task:** scale the family_destinations pattern (PR #316) from a
  single publisher to a fleet of California road-trip food articles.
  Adam supplied 7 URLs. Discovery + full AO-parity pipeline on TEST.
- **Deliverable:**
  `docs/decisions/2026-08-28-editorial-food-multi-publisher-source.md`.
- **Fetch results:** 5 of 7 URLs cleanly fetched (beyondthejourney,
  familyvacationsus, altaonline, provokelifestyle, everafterinthewoods).
  Two blocked by HTTP 403 (LA Times = anti-scrape; TasteAtlas =
  Cloudflare + wrong data shape) — dropped per Adam.
- **Design choice — one new source, not five:** picked "editorial_food"
  umbrella after weighing per-publisher source IDs (would have cost
  5×3=15 migrations) vs. renaming family_destinations (name mismatch).
  Adam signed off on option C in a mid-task check-in.
- **Row totals across 5 articles:** 80 raw. Dedupe on name+city
  showed 7 real dupes across articles + 4 rows already on TEST via
  PR #316 family_destinations. In-N-Out and Pea Soup Andersen's kept
  as separate rows (different physical locations).
- **Dataset prep** in `.context/editorial-food/`:
  - Per-article raw CSV (one per publisher).
  - `geocode.ts` — multi-CSV loop, same Mapbox two-phase strategy as
    family_destinations. **Harris Ranch overridden** (Mapbox returned
    a Cincinnati Ohio address on both articles mentioning it — added
    real Coalinga coords by hand).
  - Per-article `-geocoded.csv` output. 71 of 80 rows got coordinates;
    9 rows skipped at geocode-time when Mapbox POI didn't have them
    (Clearman's, Marin Sun Farms, El Galleon Catalina, etc.).
- **Ingester**: `data/ingestion/sources/editorial-food.ts`, mirrors
  family_destinations shape. `external_id` format
  `editorial_food:<publisher>:<article>:<row>` for triple scoping.
  `source_quality_score = 0.35` (below family_destinations' 0.4).
  `overlander_tags` include the publisher_slug + article_slug for
  future filtering. Registered in `manual.ts` as `--source editorial_food`.
- **Migrations, applied to TEST** (all via `echo y | db:push-verify -- --test`,
  the runbook fix from PR #316):
  - `20260828120000` — `field_precedence` row for
    `editorial_food` description at priority 8 (below family_destinations
    at 7) + backfill_master_place_photo_url() extension.
  - `20260828120100` — `pois_along_corridor` photo lateral extension
    to include editorial_food.
  - `20260828120200` — `master_place_search_export` view photo lateral
    extension.
- **Ingest, materialize, backfill:**
  - Ingest: 71 fetched / 71 inserted / 9 skipped / 0 errors.
  - Materialize `--only-categories restaurant --skip-sync` at
    `ER_APPLY_BATCH_SIZE=25`: **55 new_master_places, 16 manual_review**,
    0 errors, 0 auto_link. **The 0 auto_link is unexpected** — 3 rows
    (Duarte's, Gott's, Nepenthe) should have matched the
    family_destinations mps from PR #316 but instead created new mps.
    Filed in BACKLOG for investigation.
  - `backfill_master_place_photo_url` on the 55 linked mp_ids:
    **25 photo_url changed** (the other 30 either had no photo in
    the source article or the mp already carried a higher-priority
    photo).
  - `search:sync` corpus-wide against `places_test`: 32,809 docs,
    0 failed, 0 pruned.
- **Live-verify PASSED across 4 sample corridors** via the corridor
  RPC (Malibu → LA, Big Sur ↔ Coalinga, Mojave desert, Central Coast).
  Every corridor returned editorial_food rows with clean descriptions
  and — where the article provided one — photos. Harris Ranch
  correctly surfaces on the Big Sur ↔ Coalinga route via the manual
  override coordinates.
- **Live-verify on Typesense `places_test` across 10 name probes:**
  **7/10 return an editorial_food-tagged document** (Neptune's Net,
  Apple Pan, The Mad Greek, Mel's Drive-In, Harris Ranch, Ikeda's,
  Buck Owens' Crystal Palace). **3 misses (Peggy Sue's, Nepenthe,
  Alien Fresh Jerky)** — all in manual_review, so their source_records
  exist on TEST but no linked master_place → not in the search-export
  view → not in Typesense.
- **This pass touched:**
  - 1 new source module + 1 manual.ts edit
  - 3 TEST migrations
  - 1 verify script + 1 decision doc
  - The CSV dataset in `.context/editorial-food/` (gitignored per repo
    convention).
- **Flagged for follow-up (BACKLOG):**
  - 16 rows in manual_review on TEST — same triage runbook as AO's
    tail.
  - 3 unexpected non-auto_links to existing family_destinations mps
    (Duarte's, Gott's, Nepenthe) — worth understanding whether ER
    thresholds or name-normalization dropped the match.
  - 9 rows with no coordinates (never ingested) — hand-look-up
    addresses if we want the full 80-row set.
  - Photo credit is publisher-slug, not photographer. If any of this
    heads toward PROD, revisit.
  - PROD promotion of `editorial_food` — not scoped. Runbook mirrors
    AO's from PR #314 §Part 2 if ever authorized.
