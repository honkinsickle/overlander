...
3. **`master_place.id` becomes the canonical id everywhere.** `Day.waypoints` stores this id regardless of entry surface. External ids (`google_place_id`, `ridb_id`, etc.) become fields the resolver uses internally, never the primary key downstream components branch on. Whether this fully resolves the `mp:`-prefix hydration mismatch depends on `master_place.id`'s actual format, which hasn't been confirmed — if it also carries a prefix or non-Google format, the resolver will still need an explicit normalization step between canonical id and `google_place_id` lookups.
