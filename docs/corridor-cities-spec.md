...
**Edit-time (built 2026-07-07, superseding the 2026-07-06 "deferred" decision):** the missing server-side rerouting-on-edit now exists — `recomputeDay(trip, dayId)` (`web/src/lib/trips/recompute-day.ts`) reroutes ONE day **through its stops** (start → waypoints in visit order → end, via `routeBetween`), recomputes `miles` / `driveHours` / `corridorCities` (derive + bucket over the rerouted line), and persists via `repository.applyDayDerived`. All three waypoint actions (add / remove / **reorder** — visit order changes a through-stops route) call it after their mutation. This also fixed the pre-existing miles/driveHours staleness-on-edit. Measured live: ~0.7–0.9 s per edit (one Directions call + derivation), behind the existing optimistic UI.
