...
- **Fields consumed per row:** name, lat/lng, population, admin region, and (added 2026-07-06) an **administrative-significance `tier`** precomputed from the GeoNames feature code at build time: 5 national capital (PPLC) · 4 admin1 seat (PPLA) · 3 county/borough seat (PPLA2) · 2 generic populated place · 1 city section/locality (PPLX/PPLL/PPLQ). **Tier + population together drive the prominence filter** (§2.1.2) — raw population alone mis-ranked (a 207k generic neighbor over a 97k county seat).
...
4. **Spacing.** No two nodes within `min_spacing_mi` (~50 route-miles) of each other; when candidates cluster, keep the more prominent one (tier, then population).
5. **Top-N (soft cap).** Cap intermediate nodes at `max_nodes` (4) per day, selecting by prominence (tier, then population) among spacing-valid candidates. Soft — yields to step 6's gap guarantee (see precedence below).
6. **Adaptive fallback (critical for empty overland stretches).** Guarantee at least one intermediate node per `max_gap_mi` (~150 route-miles): if no city clears `pop_floor` in a segment that long, **relax the floor** and take the most prominent (highest-tier, then highest-population) available city — even a small town — so long empty stretches (remote NV, or AK on the LA→Deadhorse run) still get corridor anchors rather than a 300-mile gap with nothing. This is why the base extract must include sub-10k towns (§2.1.1).
...
**Prominence signal — ✅ RESOLVED (2026-07-06): administrative TIER first, population within tier.** Everywhere this filter says "most prominent" it means `tier` desc (§2.1.1 scale), then population desc, then deterministic tiebreaks. Raw population alone mis-ranked on real routes: Oxnard (207k, generic) beat Ventura (97k, county seat) — the spec's own mission example — and the gap-fill fallback chained five Anchorage suburbs in 43 mi. Tier-first restores Ventura and picks admin seats (Palmer, Willows) as anchors. Measured 2026-07-06: LA→SB mission example restored; Deadhorse Day 16 pile-up 5 suburbs → 2 nodes; the 23 townless gap-days unchanged (tiers can't invent towns — §3.2 holds).

**One-fill-per-gap — ✅ RESOLVED (2026-07-06): unspaced fallback fills are one-shot.** Spacing-valid fills always shrink a gap meaningfully and may recurse — successive spaced fills legitimately CURE a long gap (the 898-mi multi-fill case). An **unspaced** fallback fill (no spacing-valid candidate existed) is a one-shot: any still-oversized sub-gap it leaves is **accepted open**, never retried. Without this, the fallback walks a candidate cluster node-by-node while never curing the real gap — the structural completion of the tier fix, covering geometries where the admin seat sits mid-cluster.
...
| `anchor_guard_mi` | 10 | Candidates within this many route-miles of the Start/End anchor are dropped (both ends). Suppresses metro-neighborhood nodes (e.g. Glendale@6 out of LA); tuned 2026-07-06 — margin-safe below legit near-anchor cities at ~16 mi (Sacramento, Kalispell), which 15 clears by only 1 mi and 20 wrongly deletes. **Watch-item (2026-07-06):** metro-adjacent real municipalities just past the guard (Santa Monica@14 on LA→SB, surfaced by tier-first ranking) are KEPT on principle — same call as 10-over-20 (don't suppress real cities near metros). Revisit only if they read as clutter across more routes. |
...
### 3.1 When it's computed — FINALIZE + EDIT-TIME (edit recompute BUILT 2026-07-07, Phase 0 of the editable-corridor integration)

**Finalize (built 2026-07-06):** `corridorCities` is computed in `buildRouteAwareDays` (`web/src/lib/plan/actions.ts`) — where day polylines (`DaySegment.coordinates`) and `Day.miles` / `driveHours` are written. Day values there derive from a **direct start→end route slice** (waypoints don't exist yet at finalize).

**Edit-time (built 2026-07-07, superseding the 2026-07-06 "deferred" decision):** the missing server-side rerouting-on-edit now exists — `recomputeDay(trip, dayId)` (`web/src/lib/trips/recompute-day.ts`) reroutes ONE day **through its stops** (start → waypoints in **geographic order** — projected along the start→end chord, NOT array order — → end, via `routeBetween`), recomputes `miles` / `driveHours` / `corridorCities` (derive + bucket over the rerouted line), and persists via `repository.applyDayDerived`. Geographic stop ordering is the routing half of the Phase 3 "geography is the order / no reorder" model: adds append to the waypoint array, and insertion-order routing sent a downtown-LA add AFTER a Utah-border stop (385-mi day → 1,136 mi). All three waypoint actions (add / remove / **reorder** — visit order changes a through-stops route) call it after their mutation. This also fixed the pre-existing miles/driveHours staleness-on-edit. Measured live: ~0.7–0.9 s per edit (one Directions call + derivation), behind the existing optimistic UI.

**Semantics note — through-stops is new:** before Phase 0, NOTHING routed through waypoints (finalize routes start→end; the map drew day-endpoint routes). First edit-recompute of a day can therefore legitimately jump `miles`/`driveHours` from direct-route to through-stops values — the truthful ones. The map's route line now also threads waypoints (MapColumn), keeping display and persisted values coherent.

**Rulings (2026-07-07):**
- **Decoupled:** recompute is best-effort AFTER the edit persists — Mapbox/derivation failure logs and leaves derived values stale; it never blocks or rolls back the edit.
- **Stale corridor is replaced, not kept:** when the corridor can't be re-derived (unsplittable label, degenerate spine), `corridorCities` is cleared — a spine describing the pre-edit route is wrong; clients fall back per decision F.
- **Scope:** applies to user (UUID) AND slug/fixture trips.
- **Map display:** handled display-side (MapColumn routes through waypoints); rerouted day polylines are used transiently for derivation and NOT persisted.
- Recompute is per-affected-day, not whole-trip; skipped (null) for days with < 2 routable stops or > 25 (Mapbox coord cap).
