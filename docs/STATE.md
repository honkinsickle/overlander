# STATE — `main` · 2026-08-26 (photo pipeline LIVE ON PROD) (**newest truth: the full photo pipeline — NPS campground/park photos (PR #298), Wikipedia photo matching (PR #299), corpus tile refresh action — is DEPLOYED AND BACKFILLED ON PROD.** PROD migration waterline advanced from `20260820120000` to `20260826160100` (11 migrations). Backfills applied: NPS campground/park (198 source_records), Wikipedia (783 source_records, 541 high / 242 medium confidence), master_place.photo_url (4,439 rows — nps 2,734 + ridb 888 + wikipedia 747 + state_parks 70). "Refresh trip data" action (day kebab menu) is live — strips + re-folds corpus tiles from current RPC, no LLM cost. New trips generated on PROD will bake with current photos. Existing trips need the refresh action or a re-generation to pick up the new data.)
