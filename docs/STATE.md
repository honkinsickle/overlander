# STATE — branch `wire-web-tests-into-ci` (pushed as `surface-pop2`) · 2026-09-03 (later 23) — **CI now RUNS the web test suite. 714 tests that never executed automatically now do — but the job is NOT YET a required check, so it reports without blocking.**
...
**⚠️ THE JOB RUNS BUT DOES NOT YET BLOCK — ADAM'S ACTION, NOT A CODE CHANGE.** `main`'s protection is a **ruleset** (id `19629589`), not classic branch protection, and its `required_status_checks` names exactly three contexts: **`typecheck`, `test`, `build`** `[read from the GitHub API 2026-09-03]`. **`test-web` is not among them**, so a failing web suite shows red on the PR and still permits merge. This is the same "a check that cannot fail is not evidence" class the STANDING RULES warn about, and I wrote the first version of this masthead without checking the enforcement layer at all. **Fix: add `test-web` to the ruleset's required checks** (dashboard/API, Adam owns gates). Fallback if that is unwanted: move the step into the already-required `test` job and accept the shared-Supabase serialization.
