# Branch State — `main` (STEP 2–4 LIVE on PROD; edit UI still dark — flag unset)

```
Branch:      main   (both feature arcs merged in; working branch is now main)
HEAD/main:   964404d (STEP 4) — origin/main == local main, PUSHED & deployed green
                          (Vercel "success"). STEP 3 (`ede50c9`) + STEP 4 (`964404d`)
                          fast-forwarded 2bcf1bb→964404d. This STATE.md commit sits
                          one above 964404d (STATE-only). The UUID node-edit SERVER
                          path is now LIVE on PROD under RLS, but DARK — the edit UI
                          flag `NEXT_PUBLIC_LIVING_PLAN_EDIT` is still UNSET in Vercel.
                          History: … → 95c5c07 (STEP 2, on PROD) → 2bcf1bb (STATE) →
                          ede50c9 (STEP 3) → 964404d (STEP 4) — all now on origin/main.
...
- `964404d` — **STEP 4: ADR §1 dispatch of node-edit writes. LIVE on PROD under RLS (deployed green); edit UI DARK (flag unset).** `node-actions` (`createNodeSeed`/`pin`/`unpin`/`removeSeed`/`setPlaceRank`) now dispatch on `isUserTrip`: UUID → `public.trips` via `updateUserTripPayload` (RLS/SSR, STEP 2 envelope, `onConflict:"retry"`; the mutate re-runs the pure edit INSIDE the closure per retry so concurrent edits to different places COMPOSE, never clobber); slug → `reference_trips` service path UNCHANGED. **Bake-at-write** on the UUID path (`resolveCorridorCities` in the closure) because `getUserTrip` does NOT re-derive corridors; slug stays strip-and-serve-re-derive. **Payload shape diverges by table by design** (UUID baked / slug stripped) — each correct for its serve path, documented on `writeEdit`/`persistSlug`. Guard swap on UUID: authenticated (`getUser`) + RLS ownership (no app check); `checkRails` incl. `checkNotFrozen` intact on slug. UI gate `canEdit = !isReference && isUserTrip(trip.id)` on both edit surfaces across 4 render surfaces (references incl. Cassiar never show it). No-polyline edits persist the overlay and leave the spine unchanged (graceful). New `verify-trip-step4.ts`: 5/5 under the seeded JWT (bake-visible-in-spine, concurrency-compose, RLS refusal, frozen-slug refusal, no-polyline graceful); `next build` exit 0; 99 itinerary tests.
- **PROD EXPOSURE — STEP 1 of 2 DONE.** The push (964404d, deployed green) made the UUID node-edit **SERVER** path LIVE on PROD **under RLS** — reachable but DARK: the Vercel flag `NEXT_PUBLIC_LIVING_PLAN_EDIT` only gates the **UI** toggle, and it is still UNSET, so nothing renders it. Same model as the shipped waypoint add/remove actions (RLS-guarded, ungated). RLS (`trips_update_owner`) + `checkNotFrozen`(slug) make this pre-flag server exposure safe — a user can only write their own non-frozen trip. The UI gate was confirmed by observation on TEST: seed-owner UUID trip shows the Edit toggle; a Cassiar-shaped slug hides it, flag ON. **NEXT (Adam, separate): flip `NEXT_PUBLIC_LIVING_PLAN_EDIT` in Vercel** — step 2 of 2, turns the edit UI on.

## In-flight (uncommitted working tree)
None. STEP 2–4 are on origin/main and live on PROD (STEP 4 server path under RLS, UI dark). This STATE.md commit (STEP 3+4 pushed) sits one above 964404d. Remaining exposure step — the Vercel flag flip — is Adam's, separate.
