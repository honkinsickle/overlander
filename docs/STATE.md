...
# STATE — branch `potw-from-sheet` · 2026-09-14 (latest) — **Pin of the Day posts can now be built straight from a Google Sheet, no database.** `npm run -w data potw:sheet -- --sheet <url>` reads `photo_url | place | category | state | country`, composites each row with the existing brand frame, captions it with the 12 templates rotated by row order, and writes `pin-of-the-week/output/sheet/<slug>/{image.png,caption.txt}` plus a self-contained `review.html` (images embedded). Stacked on `fix-caption-template-typos` (PR #438).

(**newest truth: new `data/pin-of-the-week/from-sheet.ts` + `from-sheet.test.ts`, `potw:sheet` script in `data/package.json`, and `regionLine(state, country?)` in `image-prompt.ts` — with a country it uses the sheet's country instead of assuming USA; without one, behaviour is unchanged (tested).** Country appears on the image only, not in caption text (Adam's call). No Supabase read or write, nothing marked used, no post history.

**Verified** `[literal — ran this session]`: `npm run -w data typecheck` exit 0; `pin-of-the-week/` tests 8 files / 71 passed. Real run against Adam's sheet (1 row, McArthur Falls State Park, local photo path with shell escapes) built cleanly; image read by eye (photo, frame, title, "California, USA"); the review page's embedded image is byte-identical to `image.png`.

**Known limits:** the brand frame `brand/header.png` has **"Campground" + tent icon baked in**, so every post says Campground regardless of the category column. Template rotation restarts at 1 each run (no history).
...
**Verified** `[measured 2026-09-13]`: branch file in the simulator — card top `5` at scrollY 0, max scroll 150 (was 175, page 25px shorter). Chromium 402px — `.hero` padding-top `5px`, card top `5`. Chromium 1024px — media query false, padding-top `0px`, `.card-wrap` `absolute`, card top `184.31` (same as before the change). Not verified on a physical iPhone.
...
**Follow-up fix (found by dogfooding the skill)** `[literal — CLI now returns `committed: true` where it twice failed]`: making the first real post (Cape Blanco State Park, OR — a no-corpus-photo place rescued by a `--file` override) exposed that **`potw:select --commit` never consulted the photo override** and so refused to mark used any override-only-eligible place (every no-corpus-photo place). Only `generate` applied it. Fixed by extracting a shared `applyPhotoOverride()` in `photo-override.ts` that BOTH `generate` and `select`'s manual path call (can't drift again) + 4 unit tests. Cape Blanco is posted (post #2, archived on disk, `featured_at` stamped).

**Merge state** `[literal — `git grep applyPhotoOverride origin/main` returns nothing]`: **PR #424** (branch `build-pin-of-the-day-skill` — Conductor renamed `louisville` → this before the first commit) squash-merged to `main` (`37985a0`) but **captured only the FIRST commit (the build); the fix `d9f6424` was NOT in the squash** — a merge/push race. So **`main` currently carries the override-commit bug.** The fix goes to `main` via a SEPARATE follow-up PR from branch `fix-potw-select-override` (cherry-pick of `d9f6424`, gates green). **NOT DONE / NEXT:** Adam reviews + merges the follow-up PR after CI. Apply the migration to PROD only when the pipeline itself goes to PROD (still TEST-only today). Optionally commit the Cape Blanco post archive (`data/pin-of-the-week/posts/2026-09-11-cape-blanco-state-park/`) — a real approved post. No PROD anything. Design record marked BUILT.
...
**Q1 — Culture → `attraction`, and the code says so in prose** `[literal]`: `federated.ts:42` reads *"attraction: the formal cultural set only"*, with the adjacent comment putting generic `tourist_attraction` in `oddity` *"NOT in the formal-cultural attraction bucket."* **The corpus taxonomy already draws exactly the distinction Adam is drawing** — Culture isn't a new concept being placed, it's the existing definition of `attraction` given a label and chips. `oddity` was never a candidate (Atlas Obscura, 2,745 rows, *curious* not *cultural*).

**⚠️ THE #380 DIAGNOSIS GOT SHARPER, NOT JUST CONFIRMED.** #380 reported a corpus-vs-live disagreement and rated it `[unverified]` whether deliberate. Re-checking found a **third** encoding: Foursquare's classifier (`foursquare.ts:76-82`) also files museum/gallery/historic under `attraction`, commented *"mirrors the federated corpus split."* With Google's fanout that's **three against one** — `LIVE_SLIDE_FOR_PRIMARY` is the sole outlier, which is strong inference it's a slip. Fix is **three lines in one constant.**

**⚠️ THEATERS: THE PRIMARY DOESN'T EXIST, AND SOMETHING ELSE ALREADY CLAIMS THE WORD** `[literal]`. No `theater`/`theatre` primary anywhere — not in the 70 TEST corpus values, not in `web/src` or `data/`. But `foursquare.ts:84-89` **deliberately** routes `theater|theatre` → `oddity` (*"Roadside-quirky entertainment stays oddity"*). Ship Culture without touching it and FSQ theater results land in `oddity` while the Theaters chip queries `attraction` — **recreating, for theaters, exactly the split we just closed.** Flagged as a product question: that rule may have meant drive-ins, whereas Adam's "Theaters" reads as performing-arts. `[unverified]` Mapbox theater coverage — no stored id list in the repo, needs a live probe.

**Q2 — my proposal for `interest`, one option not a menu:** keep it as one of the 9 and keep its chip, but change what it means — **the fallback sink, drained of everything with a better home, rendering no subtype chips.** Promote `car_repair`/`car_wash`/`rest_area` → `fuel`, `amphitheatre` → `attraction`, `marina` → `scenic`, `hut` → `overnight`, `picnic_area` → `scenic`. Keep the parent chip (it holds `facility` at 2,245 in-scope and is the `primaryCategoryToSlideKey` fallback). Rename it off "POINT OF INTEREST."

**The decisive argument for Auto/Repair → `fuel`: the UI already asserts it and only the data model disagrees** `[literal]` — the Find Nearby group heading is literally **"FUEL & REPAIR."** Nobody needs persuading that a mechanic belongs near a gas station.

**A new Services cluster under `fuel`** takes auto/repair, rest areas, and — replacing #380's own weaker proposal — water fill, showers, dump stations, toilets. **`camping` was rejected with reasoning:** campground amenities are *attributes of a campground*; a dump station you drive to is a **service stop**, the same errand as fuel and laundry.

**Why not split `interest` into finer subtypes: because its mass isn't splittable.** `facility` is 2,245 in-scope and `[cited #364]` *"a generic RIDB container spanning campgrounds, day-use sites and offices."* No taxonomy decision splits that — only richer ingest. Inventing chips over it would produce filters returning arbitrary subsets. **This fix makes `interest` honest, not good; the residue is still a residue.**

**⚠️ THE RISK THIS AMENDMENT INTRODUCES, NAMED RATHER THAN BURIED: two of Culture's four chips are empty and a third doesn't exist.** Museums and Galleries have **0** corpus rows; `theater` has no primary. Only **Historic Sites** has corpus (`historic` 24, `landmark` 3, plus one row each of `historical_landmark`/`historical_place`/`monument` `[literal]`) — and it also has the best-measured live coverage in the routing table (6/6 metro, 6/6 rural). **A cluster whose chips all return nothing is worse than no cluster.** Ship Historic Sites first, Theaters last.

**Also flagged: `oddity` loses live results when the fix lands, and that's intended** — Google emits nothing for it by design and Mapbox is fuel-only, so its bbox live half goes to zero. Correct for a corpus-primary category on 2,745 rows, but expect it rather than discover it.
...
# STATE — branch `main` · 2026-09-02 (later 8) — NV session wrap: PR #349 merged, doc pass (**newest truth: PR #349 (`nevada_state_parks`) squash-merged to `main` as `1b1a912` at 07:02:35Z. The three pre-squash commits on `nv-state-parks-investigation` (`eeb12a0` ingest, `9caf2f6` triage, `e333476` LV Mormon Fort workaround) are collapsed; local branch still holds them at `e333476..origin/main`, safe to delete at Adam's discretion. Working tree clean on `main`. **On-main state after this session:** `nevada_state_parks` live on TEST, 28/28 source_records / 4 new master_places / 28 confirmed place_matches / 3 migrations (`20260902003000/003100/003200`). Photo laterals at priority 9 in both `pois_along_corridor` and `master_place_search_export`. Old LV Mormon Fort mp (`d331abb7…`) manually corrected to `is_searchable=true`/`primary_category='historic'` — one-shot workaround for the systemic `recompute_master_place` gap that's still open in BACKLOG. **One rebase-conflict resolved before merge:** #348 (docs-only wrap for #346) landed on main while PR #349 was open, editing STATE.md + LOG.md; rebased atop `d8130c7`, kept both sides in each file (NV mastheads/entries above #348's), re-ran full gate (data typecheck / web typecheck / next build all 0), force-pushed with `--force-with-lease`, merged shortly after. **PRs #350/#351 (AZ) landed on main AFTER #349 and are NOT this session's work** — recorded here only to explain why `origin/main` now sits 2 commits ahead of the merged NV state (the AZ masthead immediately below documents that work). **Two follow-ups remain open, unchanged by this wrap:** (a) upstream `sp_extract.py` scraper bug — `fees` field captured as site nav-menu on all 28 NV rows, raw parked in `provenance.fees_raw`, never surfaced, needs upstream fix + re-scrape; (b) systemic `recompute_master_place` gap (fixed for Old LV Mormon Fort only). No decision doc filed — the two documented divergences from CA/WA/OR precedent (fees deferral, photo-license `"Nevada State Parks"` vs `"— government publication"`) live in PR body + BACKLOG + ingester file-header; deferred until Adam signals worth the formal record. **Stale lines in earlier mastheads (preserved verbatim per file convention, called out here):** `later-2 (nv)` ends "PR to be opened next" (opened as #349, merged), `later-4` ends "PR #349 remains open, awaiting merge — do NOT self-merge" (merged). No PROD work this session; PROD promotion requires Adam's explicit sign-off per standing rules.**)
...
# STATE — branch `where-my-photo` · 2026-09-01 (later) (State-park photo sourcing SCOPED for all six states — none is license-clear; NOTHING BUILT) (**newest truth: photos from each state's own official state-parks site were scoped for CA/AZ/NV/UT/WA/OR, and NONE is unblocked with clear terms, so no ingestion/normalizer mapping was built — per the task's "report blocked rather than scrape" rule.** Enumerate-before-measure: grounded ArcGIS field enumeration + live-value samples (read-only, `data/scripts/investigate-stateparks-photo-{fields,values}-2026-09-01.ts`), then six parallel per-state licensing-research passes with cited terms. **Part 1 (measured):** WA is the ONLY state whose ingested ArcGIS layer exposes a usable image URL — `Imagelink` → real `parks.wa.gov` JPGs (138 non-empty in the live layer). AZ `PHOTO` = internal codes (`PTL_0001`) with `hasAttachments:false` (verified, not resolvable attachments); NV `photo` = empty `" "` placeholder; UT `weblink1` = park **webpage** links, not images; CA + OR carry no image field at all (`hasAttachments:false`). **No layer carries any caption/credit/license field.** **Part 2 (cited terms, blocked everywhere):** CA — parks.ca.gov personal/non-commercial only, no deep-link/spider, no cropping; AZ — "not in the public domain," all rights reserved; NV — "educational and scientific purposes" only + gallery images are third-party Instagram works; UT — third-party SmugMug, no reuse grant; **WA — "© … All rights reserved," no third-party grant, no attribution term, and some images are contributed third-party (Foundation photo contest / "used with permission") works `Imagelink` can't disambiguate → do NOT map to display on this basis**; OR — mandatory "courtesy Oregon Parks and Recreation Department" credit + "may be subject to copyright," web-only/no machine-readable source → blocked for a pipeline. **The standing "official agency/park media is license-clear" assumption is FALSIFIED for these six — and fails hardest exactly where usable data exists (WA).** **Retroactive flag:** the prior WA backfill left ~59–70 `parks.wa.gov` URLs in `master_place.photo_url` (measured PROD: `ILIKE '%parks.wa.gov%'` = 70; 59 active Imagelink records; **parks.ca.gov = 0**) with no confirmed reuse grant — not rendered (laterals exclude state_parks), latent, should not be wired to display; Adam may want them cleared (not deleted here). **No code/data/schema/PROD change** — normalizer unchanged, render laterals unchanged. Report-only per the task. ADR: `docs/decisions/2026-09-01-state-park-photos-not-license-clear.md`; scope: `docs/measurements/2026-09-01-state-park-photo-sourcing-scope.md`. Stacked on the MacKerricher masthead below (same `where-my-photo` branch / PR #343 thread). Gate: `npm run -w data typecheck` exit 0. The masthead immediately below is preserved verbatim per this file's convention.)
# STATE — branch `where-my-photo` · 2026-09-01 (MacKerricher "missing photo attribution" traced — it's a live Google Places photo, attribution structurally dropped; INVESTIGATION ONLY) (**newest truth: the MacKerricher State Park card's uncredited photo is a LIVE Google Places photo, not an ingested corpus photo, and the corpus carries no MacKerricher photo at all.** Read-only, PROD + TEST, nothing modified. The card is a baked Google-resolved LLM key-stop tile ("MacKerricher State **Park**", `placeId ChIJrSiZmdI0gIARBI706DgDanc`, no baked `photoUrl`) in PROD user trip `63634fd5-203c-43b1-b61a-35d286c1ccc2`; `day-detail-corridor-column.tsx:308-351` live-hydrates any `placeId && !photoUrl` tile via `/api/places/details` → `placeDetails()` → `/api/places/photo`. `placeDetails()` extracts only `photos[0].name`; `GooglePlace.photos` is typed `Array<{ name: string }>`, so Google's `authorAttributions` are **structurally discarded** and `PlaceRich` has no credit field (`photoCredit` only ever comes from CC federated `row.photo_credit`). → **a Google Places Platform attribution-display gap, LIVE on PROD, affecting every live-hydrated Google photo** — NOT a corpus license-clear violation and NOT the baked `photoUrl` path. Both PROD `master_place` MacKerricher rows ("MacKerricher SP" recreation_area, "MacKerricher Walk-in Camp" campground) are `state_parks`-only with `photo_url` NULL / export-view lateral NULL / no google id; TEST matches (plus a `public_land` shell + `generated_template` description). **parks.ca.gov photos ingested: ZERO — measured** (of 1,736 PROD `state_parks` records, all 572 CA rows carry 0 `Imagelink`; the only 59 populated `Imagelink`s are all `parks.wa.gov`; `master_place.photo_url` 7,223 non-null → 70 `parks.wa.gov`, **0 parks.ca.gov**). Those ~59–70 WA images are hotlinked with no attribution but **not rendered** (laterals exclude state_parks) → latent, filed. The BLM/state_parks unmapped-photo thread (`20260821070000` header) does NOT affect MacKerricher (no image field to map). Report only per the task — no app/data/schema/PROD write, env never swapped. Two BACKLOG items filed (live Google-attribution gap; latent WA hotlink gap). Scripts: `data/scripts/investigate-mackerricher-{photo,baked-tile}-2026-09-01.ts` (read-only, ref-guarded). Full trace: `docs/measurements/2026-09-01-mackerricher-photo-attribution.md`. **Branch note:** this session runs on `where-my-photo` (clean tree at start); STATE's prior masthead was branch `missing-photos`. The masthead immediately below is preserved verbatim per this file's convention.)
