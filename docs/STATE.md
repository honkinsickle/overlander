# Branch State — `main` (STEP 2 LIVE on PROD; STEP 3 + STEP 4 committed, unpushed)

```
Branch:      main   (both feature arcs merged in; working branch is now main)
HEAD/main:   STEP 4 (ADR §1 dispatch) — local main is 2 commits AHEAD of
                          origin/main (`2bcf1bb`, STEP-2-live). STEP 3 (`ede50c9`)
                          + STEP 4 (this commit) are TEST-only, UNPUSHED; Adam
                          authorizes the push. STATE.md is written against `ede50c9`;
                          the STEP 4 commit carrying it sits one above.
                          History: e51e8be → d56840d (checkNotFrozen rail) →
                          3e909da (STEP 1) → 95c5c07 (STEP 2, on PROD) → 2bcf1bb
                          (STATE: STEP 2 live) → ede50c9 (STEP 3) → THIS (STEP 4).
