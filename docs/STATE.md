# STATE — branch `feat/editorial-food-multi-source` · 2026-08-28 (later) (editorial_food multi-publisher source on TEST) (**newest truth: a NEW source `editorial_food` landed end-to-end on TEST, scaling PR #316's `family_destinations` pattern from one publisher to a fleet of 5 California road-trip food articles.** Content: 5 articles from beyondthejourney (21), familyvacationsus (16), altaonline (5), provokelifestyle (27), everafterinthewoods (11) = 80 raw rows; 71 with coordinates (89%) after Mapbox two-phase geocode + 1 hand override (Harris Ranch, which Mapbox misplaced in Ohio); 9 rows skipped at geocode-time (hyperlocal not in Mapbox POI). Pipeline: fetch → extract → geocode in `.context/editorial-food/`; new ingester `data/ingestion/sources/editorial-food.ts` (globs *-geocoded.csv, source_quality_score 0.35, external_id `editorial_food:<publisher>:<article>:<row>`); registered in `manual.ts` as `--source editorial_food`; 3 TEST migrations (`20260828120000` field_precedence + backfill_master_place_photo_url extension at priority 8, `20260828120100` pois_along_corridor extension, `20260828120200` search-export view extension) all applied cleanly via `echo y | db:push-verify -- --test`. **TEST results:** ingest 71/71 inserted 0-errors; materialize `--only-categories restaurant --skip-sync` at `ER_APPLY_BATCH_SIZE=25` yielded 55 new_master_places + 16 manual_review + 0 auto_link + 0 errors (**the 0 auto_link is unexpected** — 3 rows should have matched existing family_destinations mps from PR #316; filed in BACKLOG); `backfill_master_place_photo_url` on the 55 linked mp_ids reported 25 changed; `search:sync` corpus-wide against `places_test`. **Live-verify PASSED on TEST via corridor RPC across 4 sample corridors** (Malibu → LA, Big Sur ↔ Coalinga, Mojave, Central Coast) — every corridor returned editorial_food rows with clean descriptions + (mostly) photos; Harris Ranch surfaces correctly via the manual override. **Typesense probes: 7/10 return the editorial_food-tagged document**; 3 misses (Peggy Sue's, Nepenthe, Alien Fresh Jerky) are all in manual_review. **Test-only per Adam's 2026-08-28 directive. PROD not touched. PROD promotion NOT scoped this session** — if ever authorized, runbook mirrors AO's from PR #314 §Part 2 verbatim. Discovery pass dropped LA Times (HTTP 403) and TasteAtlas (HTTP 403 + wrong data shape). New decision doc: `docs/decisions/2026-08-28-editorial-food-multi-publisher-source.md`. Full narrative: `docs/LOG.md` §2026-08-28 (later). Base branch `feat/family-destinations-food-test-run` (PR #316) — this PR depends on that one merging first. The masthead below is preserved verbatim per this file's convention.)
