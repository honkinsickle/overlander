...
**Follow-up fix in the same PR (found by dogfooding the skill)** `[literal — CLI now returns `committed: true` where it twice failed]`: making the first real post (Cape Blanco State Park, OR — a no-corpus-photo place rescued by a `--file` override) exposed that **`potw:select --commit` never consulted the photo override** and so refused to mark used any override-only-eligible place (every no-corpus-photo place). Only `generate` applied it. Fixed by extracting a shared `applyPhotoOverride()` in `photo-override.ts` that BOTH `generate` and `select`'s manual path call (can't drift again) + 4 unit tests. Cape Blanco is posted (post #2, archived on disk, `featured_at` stamped).

**PR #424** (branch `build-pin-of-the-day-skill` — Conductor renamed `louisville` → this before the first commit; same branch, same work) carries both the build and this fix. **NOT DONE / NEXT:** Adam reviews + merges #424 after CI; apply the migration to PROD only when the pipeline itself goes to PROD (still TEST-only today). Optionally commit the Cape Blanco post archive (`data/pin-of-the-week/posts/2026-09-11-cape-blanco-state-park/`) — a real approved post. No PROD anything. Design record marked BUILT.
