...
- The drawn geometry itself is the whole-trip `Trip.routePolyline` (encoded), decoded
  ...
  (`src/components/trip/day-detail-corridor-column.tsx:120`); the per-day polyline is
...
## 4. Scroll / windowing / map layer — **built and working; do not rebuild**

> There is **no virtualization / windowing**. The day detail is a single-day
> conditional swap, and there is **one** shared map. A continuous-scroll windowed
> variant was scoped but deliberately NOT built (see
> `docs/decisions/2026-07-24-cross-day-stop-movement.md`). Touch this only with a
> deliberate decision — the failure modes below are real.
...
- **Detail — `DayDetailCorridorColumn`** (`src/components/trip/day-detail-corridor-column.tsx:91`)
  derives ONE `day` from `selectedDayId` (`:113-115`) and renders a single
  `{day ? <DayDetailCorridor/> : <DayDetailOverview/>}` swap inside one scroll
  container (`:630-637`). The edit spine is `DayDetailNodeBlocks`
...
- **Selection ↔ scroll.** Selection is URL state: `selectDay` writes `?day=` via
  ...
  IntersectionObserver** (`src/components/trip/day-detail-corridor-column.tsx:334-364`;
  the comment at `:337` explains why IO's "topmost intersecting" was rejected).
...
- **`rescopeOverlays`** (`src/lib/corridor/rescope-overlays.ts`, PR #130) — the pure
  step that DROPS overlays orphaned by a **day-structure change** (cross-day move,
  ...
  cross-day pull-in guard). It keeps survivors unchanged (never rewrites nodeIds).
  Pure; keyed on the new day layout only. First consumer is `moveWaypointToDay` (in
  flight — see the decision doc). As of this writing it is not yet wired.
...
- User trips (UUID id) live in `public.trips.payload` (jsonb), RLS-scoped; slug trips
  are in-memory fixtures / `reference_trips` (`src/lib/trips/repository.ts:78-85`).
...
- **Two collapse patterns:**
  - **STEP 3 (geometry):** precompute the Mapbox recompute OUTSIDE the write
    (`deriveAfterDayEdit` → `recomputeDay`, `src/lib/trips/repository.ts:38-57`,
    `src/lib/trips/recompute-day.ts:48-86`), apply the precomputed `DayDerived` INSIDE
    the closure (`applyDerivedToDay`, `src/lib/trips/repository.ts:63-68`) and clear
    `routePolyline`. Used by `addWaypoint` / `removeWaypoint`
    (`src/lib/trips/repository.ts:199-295`), `onConflict: "retry"`.
