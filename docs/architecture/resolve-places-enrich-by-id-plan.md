...
`enrichPlaces`, not in `enrichByGoogleId`, which receives ids directly. So a non-Google id
passed to `enrichByGoogleId` is not filtered by `idFor`; it would simply resolve to `null`
from `placeDetails` (Google not-found / non-OK) and be omitted from the map — the same
null-omit path as any failed id. An implementation *may* pre-filter obviously non-Google
ids to save a wasted upstream call, but that is an optimisation, not required for
correctness — the route does not pre-filter today.)
