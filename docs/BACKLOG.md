...
**Post-merge a "tile missing on some days" report was investigated (2026-08-24)
and traced to pre-#279-deploy trips — not a code defect** (`isOvernight` was
universally absent because none of the TEST trips were generated by #279 code;
the wiring is verified correct by `bake-overnight-integration.test.ts`). See the
decision doc's Follow-up section. Remaining and still open:

- **Regenerate to confirm** — the overnight link has not yet been seen working on
  a trip generated by #279-deployed code (only unit/integration-tested).
- **Residual pool-hit id-mismatch (UNCONFIRMED)** — TEST has duplicate
  `master_place` rows per place (7 "Silver Strand" rows); a pool-hit overnight
  whose name-match picks a different row than the corridor fold would stay
  unmarked even post-#279. Needs a post-#279 generation to confirm.
